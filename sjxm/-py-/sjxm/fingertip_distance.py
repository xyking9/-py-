...
class FingertipDistanceMeasurer:
    ...
    def __init__(self):
        ...
        self._last_left_index_tip = None
        self._last_right_index_tip = None
        self._missing_left_frames = STALE_FRAME_LIMIT + 1
        self._missing_right_frames = STALE_FRAME_LIMIT + 1
    ...
    def _enhance_frame_for_detection(self, frame):
        """Improve low-light frames before hand landmark detection."""
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        mean_brightness = float(np.mean(gray))
        if mean_brightness >= LOW_LIGHT_MEAN_THRESHOLD:
            return frame
    ...
    def _apply_tip_cache(self, detected_left, detected_right):
        """Keep short detection dropouts from making fingertips disappear immediately."""
        if detected_left is not None:
            self.left_index_tip = detected_left
            self._last_left_index_tip = detected_left
            self._missing_left_frames = 0
        else:
            self._missing_left_frames += 1
            self.left_index_tip = (
                self._last_left_index_tip
                if self._missing_left_frames <= STALE_FRAME_LIMIT
                else None
            )

        if detected_right is not None:
            self.right_index_tip = detected_right
            self._last_right_index_tip = detected_right
            self._missing_right_frames = 0
        else:
            self._missing_right_frames += 1
            self.right_index_tip = (
                self._last_right_index_tip
                if self._missing_right_frames <= STALE_FRAME_LIMIT
                else None
            )
    ...
    def _process_frame(self, frame):
        ...
        h, w, _ = frame.shape
        detection_frame = self._enhance_frame_for_detection(frame)
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv2.cvtColor(detection_frame, cv2.COLOR_BGR2RGB))
        result = self.hand_landmarker.detect(mp_image)

        best_tips = {'Left': (0.0, None), 'Right': (0.0, None)}
        if result.hand_landmarks and result.handedness:
            for hand_landmarks, handedness in zip(result.hand_landmarks, result.handedness):
                ...
                if len(hand_landmarks) > 8:
                    ...
                    if hand_label in best_tips and score > best_tips[hand_label][0]:
                        best_tips[hand_label] = (score, (x, y))

        self._apply_tip_cache(best_tips['Left'][1], best_tips['Right'][1])
    ...
    def run(self):
        """程序入口：优先尝试摄像头模式，不可用时自动进入画板模式"""
        camera_index = self._find_camera()

        if camera_index == -1:
            print("未检测到可用摄像头，自动进入画板模式。")
